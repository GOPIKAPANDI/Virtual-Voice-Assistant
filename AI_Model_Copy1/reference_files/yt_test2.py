...
def execute_command(command, ip_address):
    ...
    try:
        ...
        # Check return code and print success or failure based on it
        ...
        print(f"Exception while executing command on {ip_address}: {e}")
        return False
...
def run_command_via_ssh(ip, command, username, password):
    ...
    try:
        ...
        print(f"Exception while executing SSH on {ip}: {e}")
        return False
...
def run_command_via_winrm(ip, command, username, password):
    ...
    try:
        ...
        if result.status_code == 0:
            ...
            print(f"Failure: {ip}")
            print(f"Error: {result.std_err.decode('utf-8', errors='replace')}")
            return False
    except Exception as e:
        print(f"Exception while executing WinRM on {ip}: {e}")
        return False
...
def run_command_via_dcom(ip, command):
    ...
    try:
        ...
        print(f"Failure: {ip}")
        print(f"Exception while executing DCOM on {ip}: {e}")
        return False
